-- Remove fields that user doesn't want
ALTER TABLE public.piq_forms 
DROP COLUMN IF EXISTS selection_board,
DROP COLUMN IF EXISTS batch_no, 
DROP COLUMN IF EXISTS chest_no,
DROP COLUMN IF EXISTS roll_no;

-- Add missing fields from PIQ form PDF
ALTER TABLE public.piq_forms
-- Personal Information
ADD COLUMN IF NOT EXISTS religion text,
ADD COLUMN IF NOT EXISTS caste_category text, -- SC/ST/OBC
ADD COLUMN IF NOT EXISTS mother_tongue text,
ADD COLUMN IF NOT EXISTS date_of_birth date,
ADD COLUMN IF NOT EXISTS marital_status text, -- Married/Single/Widower
ADD COLUMN IF NOT EXISTS state_and_district text,

-- Family details - Parents/Guardian education
...
-- Siblings details (allowing multiple entries as JSON)
ADD COLUMN IF NOT EXISTS siblings_details jsonb DEFAULT '[]'::jsonb,

-- Educational Record (as JSON array for multiple qualifications)
ADD COLUMN IF NOT EXISTS educational_record jsonb DEFAULT '[]'::jsonb,

-- Physical details (updating existing columns)
-- age_years and age_months already exist
-- height and weight already exist

-- Activities and Training
-- games_and_sports already exists
-- hobbies already exists
-- extra_curricular_activities already exists
-- positions_of_responsibility already exists
-- ncc_training and ncc_details already exist

-- Service preferences
-- commission_nature already exists 
-- service_choice already exists
-- previous_attempts already exists
-- previous_ssb_details already exists

-- Add summary field that contains all PIQ details as text
ADD COLUMN IF NOT EXISTS piq_summary text;

-- Create function to generate PIQ summary
...
-- Create trigger to auto-generate summary on insert/update
...
-- Create trigger
DROP TRIGGER IF EXISTS trigger_update_piq_summary ON public.piq_forms;
CREATE TRIGGER trigger_update_piq_summary
  BEFORE INSERT OR UPDATE ON public.piq_forms
  FOR EACH ROW
  EXECUTE FUNCTION update_piq_summary();